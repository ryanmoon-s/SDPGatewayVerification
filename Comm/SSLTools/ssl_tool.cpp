...
int SSLHelper::_SSL_LoadCertificate(std::string cert, std::string pri_key) 
{
    // 加载自己的证书
...
int SSLHelper::_SSL_BindSocket(int fd) 
{
...
int SSLHelper::SSLConnect(std::string ip, int port) 
{
    int ret = 0, fd;
...
int SSLHelper::SSLBindSocket(int fd) 
{
...
int SSLHelper::SSLSend(const std::string& msg) 
{
    int ret = 0;
...
int SSLHelper::SSLRecv(std::string& msg) 
{
    int ret = 0;
    char buf[BUFSIZ];
