...
private:
    ...
    // 转发网络请求至具体服务
    int _RequestForwardWithCmd(int32_t cmdid, const std::string& request, std::string& response, Header& header);
...
private:
    ...
    // Packet 解包与打包
    int _ParseDataFromString(Packet& packet, const std::string& str);
    int _PackDataToString(std::string& str, const Packet& packet);
