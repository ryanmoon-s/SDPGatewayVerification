...
int ErpcHandler::HandleUDPRequest(const FdDataType& fd_data)
{
    ...
    {
        ret = service->TestFuncUdpRecv(msg_str, from_ip, from_port);
    } 
    else if (cmdid == erpc::CMD_UDP_CONTROLLER_FUNC_RECV)
    {
        ret = service->ConFuncUdpRecv(msg_str, from_ip, from_port);
    }
    else if (cmdid == erpc::CMD_UDP_APPGATEWAY_FUNC_RECV)
    {
        ret = service->GateFuncUdpRecv(msg_str, from_ip, from_port);
    }
...
int ErpcHandler::_RequestForwardWithCmd(int32_t cmdid, const std::string& request, std::string& response, const erpc::Extra& extra)
{
    ...
    else if (cmdid == erpc::CMD_RPC_APPGATEWAY_FUNC_BLACK_LIST_OP)
    {
        TLOG_MSG(("RPC forward to cmdid:%d", cmdid));
...
int ErpcHandler::UDPSend(const std::string& outstr, const std::string& dest_ip, int dest_port)
{
    ...
    TLOG_DBG(("outstr size:%d, sent size:%d", outstr.size(), ret));
    return 0;
}
